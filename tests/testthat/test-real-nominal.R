...
#
# The simulation helpers live in helper-acmy.R, because the maximum
# likelihood tests need them too.
...
test_that("a supplied short rate removes the need for the one-month yield", {
  # Same relaxation as in atsm(): the one-month nominal point is only the
  # fallback. Surfaced by comparing against pyacm, which drops the first two
  # maturities from its factor extraction.
  s <- sim_acmy(tt = 150L)
  p <- sim_panel(s)
  cpi <- sim_cpi(s)

  expect_error(atsm_real(p, inflation = cpi, maturities = 3:120),
               "unless you supply one")

  f <- suppressWarnings(suppressMessages(
    atsm_real(p, inflation = cpi, maturities = 3:120,
